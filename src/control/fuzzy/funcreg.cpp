...
valoresFuzzy fuzzy::funcoes(float erro, float varerro)
{
    ...
    saida.VCD = max(regra1, max(regra2, regra6));

    saida.VMD = max(regra3, max(regra7, regra11));
    ...
    saida.CEN = max(regra9, max(regra13, regra17));
    ...
    saida.VME = max(regra15, max(regra19, regra23));

    saida.VCE = max(regra20, max(regra24, regra25));
